...
struct VS_OUTPUT
{
    ...
    float4 WorldPosition   : TEXCOORD3;   // vertex position 
		float3 dist : TEXCOORD4; // vertex distance to the camera (BK)
    float4 Color		: COLOR;
};
                         ...
                         float4 cInstanceColor)
{
  ...
  // Output.WorldNormal = (0.5*mul(vNormal,mWorld).xyz)+0.5.xxx;
    Output.WorldNormal = (mul(vNormal,mWorld));
		
		float4 Po;
		float3 Pw;
		if (isBillboard)
		{
			//float2 camXZ = normalize(float2(viewpoint.x, viewpoint.z));
			//float4x4 rot = float4x4(float4(camXZ.x, -camXZ.y, 0, 0), float4(0, 0, 1, 0), float4(camXZ.y, camXZ.x, 0, 0), float4(0,0,0,1));
			//Po = mul(vPos, rot);
			Po = float4(vPos.xyz,1.0);				// TEMPORARY!!!!!!!!!!!!!
		}
		else
		 Po = float4(vPos.xyz,1.0);				// object coordinates
		 
    Pw = mul(Po,mWorld).xyz;					// world coordinates
		//Pw = Po;
...
{
	//	return float4(IN.TextureUV.x,0,0,1);
		
		//return float4(1,0,0,0);
	float4 diff = tex2D(MeshTextureSampler, IN.TextureUV);
	
	if(diff.w < 0.5f) // alpha test 
	{
		clip(-1);
		return float4(0,0,0,0);
	}	
	
	if (length(IN.dist) >= distFloat && !isBillboard)
	{
		return float4(0,0,0,0);
	}

	// randomize for billboard
	if (length(IN.dist) < distFloat && isBillboard)
	{
		return float4(0,0,0,0);
	}	
	...
	// for Billboard the distance function should be randomized
	// otherwise the plane blending is well visible
