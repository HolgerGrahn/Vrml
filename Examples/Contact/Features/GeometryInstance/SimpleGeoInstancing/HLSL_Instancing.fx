                         ...
                         float4 cInstanceColor : COLOR3)
{
	...
	return SharedVS(vPos,vNormal,vTexCoord0,mInstanceMatrix,cInstanceColor);
}
...
float4 InstancedPS(VS_OUTPUT IN) : COLOR 
{
	...
	color *= IN.Color.rgb; // modulate with Instance Color
...
float4 InstancedTexturePS(VS_OUTPUT IN) : COLOR 
{
	...
	// Sample the texture
	float4 texColor = tex2D(MeshTextureSampler, IN.TextureUV);
	// Calculate the diffuse reflection
	float3 diffuse = saturate(dot(IN.WorldNormal, -light.direction)) * material.diffuseColor *light.diffuseColor;
	...
	float3 color = saturate((light.ambientColor * material.ambientColor)+diffuse)* texColor +specular+material.emissiveColor;
	color *= IN.Color.rgb; // modulate with Instance Color
	float alpha =material.diffuseColor.a+texColor.a;
